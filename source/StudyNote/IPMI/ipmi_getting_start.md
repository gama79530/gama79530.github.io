...
### 4. IPMI 命令是如何送進 BMC 的？有哪些常見的傳輸介面（interface）？
...
3. 解包：BMC 解析外層協定（如 KCS, BT, LAN 等），取得核心 IPMI message，並取出 NetFn、Command、Data 等欄位。
...
SDR 只告訴我們要如何解讀 sensor 回傳的數值讀數，在解讀之前需要通過 IPMI 的指令 (e.g. `Get Sensor Reading`) 來取得讀數。另外因為不同 sensor 的單位, 解讀方式全都不一樣，所以通常也是直接依賴工具幫忙轉換成人類可以直接讀的懂的數據。
...
System Event Log 是 IPMI 用來對系統事件或者設定資訊做非揮發性儲存的一個儲存系統。

在這個系統下有三個角色

- Event Generator devices - 負責在事件發生的時候發出 log request 。通常是由 host 或者 sensor 扮演。
- Event Receiver - 負責接收 log request 並把 log 寫到 repository 裡面。通常是在 BMC 上面會有一個程式專門處理。
- Repository - 儲存 SEL 的空間。

```{note}
1. 可以連接到儲存空間的裝置都可以視為 `SEL device` ，通常是指 `Event Receiver` 不過實際上也沒有很明確的規範禁止 Event Generator devices 直接控制 Repository ，但這種方式比較罕見。

2. IPMI 並沒有硬性規定 Event Receiver 只能有一個，不過比較常見的架構是只會在 BMC 上面有一個 Receiver 以及對應的 Repository 。
```

#### SEL Device Commands

| Command | M/O | Description |
|---|:---:|---|
| Get SEL Info | M | 回傳 SEL 已經記下的記錄數量、SEL command 版本、最近一次的刪除/清空的時間戳記 |
| Get SEL Allocation Info | O | 回傳儲存空間的資訊（可分配總單位數、可用空間、單位大小、最大連續空間） |
| Reserve SEL | O | 回傳 reservation ID |
| Get SEL Entry | M | 取得指定 Record ID 的 SEL 內容 |
| Add SEL Entry | M | 新增一筆 SEL 到 SEL repository |
| Partial Add SEL Entry | M | 分段新增 SEL entry，適用於大於單次傳輸限制的 entry |
| Delete SEL Entry | O | 刪除指定 Record ID 的 SEL entry |
| Clear SEL | M | 清空整個 SEL repository，移除所有事件紀錄 |
| Get SEL Time | M | 回傳 SEL device 的 time |
| Set SEL Time | M | 設定 SEL device 的 time |
| Get SEL Time UTC Offset | O | 取得 SEL device 的 UTC 時區偏移值 |
| Set SEL Time UTC Offset | O | 設定 SEL device 的 UTC 時區偏移值 |
| Get Auxiliary Log Status | O | 取得 Auxiliary Log 的狀態資訊 |
| Set Auxiliary Log Status | O | 設定 Auxiliary Log 的狀態資訊 |

```{note}
1. M/O: `Mandatory` or `Optional`

2. Auxiliary Log 是由 OEM 自行定義紀錄的內容，不屬於 IPMI 標準的一部分。
```

#### Reserve SEL

這個功能幫 log 加上了 `owner` 的概念，主要的目的有

- 避免刪除錯誤的記錄
- 提供機制避免在收到新事件後立即清空 SEL
- 防止在增量讀取時取得錯誤資料

理解上可以把這個功能當作是對 log 上一個暫時的鎖，這個鎖會讓這個紀錄保證存活到 Repository 下次異動(包含`新增`或者`刪除` log )完成之後。但下次異動完成之後這個鎖就會被自動解除。

```{note}
1. 在有上鎖的狀態下如果要強行刪除 log 的話可以通過提供`回傳的 reservation ID` 的方式來刪除 log 。

2. 這個功能並沒有規範要怎樣處理 race condition (e.g. 有兩個 `owner` 同時爭奪同一筆 log 所有權)。 
...
4. 會取消鎖的操作
   ...
   - 刪除一筆 SEL
   - SEL 被清空
   - SEL 裝置被`硬體` 或 `Cold Reset` 的方式重置
   - 收到一個新的 `Reserve SEL` 指令
```
...
| Range | Description |
|---|---|
| 00h - BFh | 給 standard type 使用，目前只有定義 02h |
| C0h - DFh | 保留給會由裝置自動加上 timestamp 的 OEM SEL |
| E0h - FFh | 保留給不由裝置自動加上 timestamp 的 OEM SEL |

## 五、限制、風險與 OpenBMC 關聯（整合層）

### 13. IPMI 在安全性上有哪些先天限制或歷史問題？

IPMI 的安全性設計源自 1990 年代，與現代需求有明顯落差，主要問題包括：

- **協定設計年代久遠**：IPMI LAN 協定（如 RMCP+）加密與認證機制較弱，容易被破解，甚至存在明文傳輸。
- **預設帳號與弱密碼**：許多 BMC 出廠預設帳號密碼未更改，容易被攻擊者利用。
- **權限控管不細緻**：IPMI 權限分級簡單，部分操作權限過大，容易造成誤操作或惡意利用。
- **缺乏現代安全防護**：如`多因素認證`、`強制密碼策略`、`API 安全`等現代防護措施缺失。
- **網路暴露風險**：IPMI LAN 通常直接暴露於管理網路，若未妥善隔離，容易被遠端攻擊。
- **日誌追蹤有限**：操作紀錄有限，難以追蹤惡意操作來源。

因此，IPMI 雖然仍廣泛使用，但安全性已不符合現代標準，逐漸被 Redfish 等新協定取代。

### 14. 在 OpenBMC 裡，IPMI 和 D-Bus 的關係是什麼？

IPMI 在 OpenBMC 中是一套抽象的管理協定，定義了伺服器管理的指令與資料格式；而 D-Bus 則是 OpenBMC 內部服務之間的訊息傳遞機制，負責讓不同元件與服務溝通。

可以把這個關係想像成郵局櫃台 (IPMI) 和郵政系統 (D-Bus)：

- 郵局櫃台 (IPMI)負責接收外部客戶的需求（指令），
- 郵政系統 (D-Bus)則負責把這些需求分送到內部各部門 (底層服務) 處理。

這樣的設計有幾個好處：

- 模組解耦：IPMI handler 只需處理協定與指令，不需直接操作硬體，透過 D-Bus 呼叫底層服務，讓協定層與功能層分離。
- 底層管道可抽換：如果底層硬體或服務有變動，只需更換 D-Bus 上的服務實現，不必修改 IPMI handler，維護與擴充都更容易。
- 多協定共用底層：不只 IPMI，Redfish, Web UI 等現代協定也都透過 D-Bus 存取底層資源，讓 OpenBMC 可以同時支援多種協定。

### 15. 為什麼在 OpenBMC 專案中，很多新功能不再優先設計成 IPMI？

主要原因如下：

- **安全性不足**：IPMI 協定設計落後，無法滿足現代資安需求，容易被攻擊。
- **資料格式彈性差**：IPMI 使用二進位封包，資料結構僵硬，不易擴充與解析；現代協定如 Redfish 使用 JSON，易於擴展與交換。
- **現代管理需求**：現代伺服器管理需要 `RESTful API`, `Web UI`, `多層權限`, `事件推播`等功能，IPMI 難以支援。
- **社群與生態系轉移**：OpenBMC 社群與硬體廠商逐漸將重心移向 Redfish 等新協定，IPMI 只作為備援或相容用途。

因此，OpenBMC 新功能多優先採用 Redfish 等現代協定，僅保留 IPMI 作為相容或備援方案。
